...
func update_follower(delta: float):
	...
	if follower_player:
		follower_player.sprite_2D.flip_h = true if not active_player.right else false
		if follower_player.name == "player1":
			follower_player.sprite_2D.position.x = 16  if not active_player.right and not anim_name == "Slide" else 0
			follower_player.sprite_2D.position.x = -20 if anim_name == "Slide" else 0
	...
	if active_player.name == "player2":
		...
		if active_player.animation_player.current_animation == "Slide":
			...
			if is_flipped:
				...
				active_player.sprite1.position.x = flip_offset
	if follower_player.name == "player1" and anim_name == "Slide":
			follower_player.sprite_2D.flip_h = follower_player.right
			if follower_player.right:
				follower_player.sprite_2D.position.x = -20
			else:
...
func update_animations(player1, player2):
	...
	for player in player_list:
		...
		if player.name == "player1" and anim_name != "Slide":
			if not player.right:
				player.sprite_2D.position.x = -20
			else:
				player.sprite_2D.position.x = 16
		if player.name == "player1" and anim_name == "Slide":
			player.sprite_2D.flip_h = player.right
			if player.right:
				player.sprite_2D.position.x = -20
			else:
				player.sprite_2D.position.x = 0
		if player.sprite1:
...
func game_over():
	...
	sceneManager.change_scene(root, "game_over")
